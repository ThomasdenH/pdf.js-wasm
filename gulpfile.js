...
/**
 * The trick below inlines the wasm because webpack doesn't support synchonous
 * loading yet. The trick comes from https://github.com/yever/rusty-typescript.
 */

/** The location to where the wasm was exported. */
const WASM_PACK_DEST = 'pkg';
/** The filename of the wasm bg file. */
const BG_WASM_FILENAME = 'pdfjs_bg.wasm';
/** The filename of the js bg file. */
const BG_JS_FILENAME = 'pdfjs_bg.js';
const JS_FILENAME = 'pdfjs.js';
const D_TS_FILE = 'pdfjs.d.ts';

const WASM_DEST_DIR = 'src/wasm';
...
gulp.task('wasm', function (done) {
  ...
  gulp.src([
    ...
    path.resolve(__dirname, WASM_PACK_DEST, D_TS_FILE)
  ])
    .pipe(
      replace(
        /const { TextDecoder } = .*\n/g,
...
function inlineWasm() {
  ...
  return gulp.src([path.resolve(__dirname, WASM_PACK_DEST, BG_JS_FILENAME)])
